 ...
 */ export const warnOnInvalidCustomViews = (collection)=>{
    ...
    for (const [key, view] of Object.entries(views)){
        ...
        if (view && typeof view === 'object' && 'Component' in view && !('path' in view)) {
            console.warn(`[Payload] Custom collection view "${key}" in collection "${collection.slug}" is missing a "path" property. The view will never be rendered.`);
        }
        if (view && typeof view === 'object' && 'path' in view && !('Component' in view)) {
            console.warn(`[Payload] Custom collection view "${key}" in collection "${collection.slug}" has a "path" but is missing a "Component". The view will never be rendered.`);
        }
    }
};
